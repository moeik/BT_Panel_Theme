...
Install_Configure(){
	if [ "${version}" == "5.1" ];then
		...
		if [ "${PM}" = "yum" ]; then
			...
			python36Check=$(rpm -q python36|grep "not install")
			if [ "${python36Check}" ];then
				yum install https://kojipkgs.fedoraproject.org/packages/python36/3.6.8/1.el7/$(rpm --eval %_arch)/python36{,-libs}-3.6.8-1.el7.$(rpm --eval %_arch).rpm -y
			fi
