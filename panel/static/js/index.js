...
$(function(){
	$(".mem-release").hover(function(){
		...
		$(this).find(".mask").css({"color":"black"});
...
function ReMemory(){
	setTimeout(function(){
		$(".mem-release").find('.mask').css({'color':'black','font-size':'14px'}).html('<span style="display:none">1</span>'+lan.index.memre_ok_0+' <img src="/static/img/ings.gif">');
		$.post('/system?action=ReMemory','',function(rdata){
			...
			$(".mem-release").find('.mask').css({'color':'black','font-size':'14px'}).html("<span style='display:none'>"+percent+"</span>"+lan.index.memre_ok);
			setCookie("mem-before",memText);
			var memNull = getCookie("memRealUsed") - rdata.memRealUsed;
			setTimeout(function(){
				if(memNull > 0){
					$(".mem-release").find('.mask').css({'color':'black','font-size':'14px','line-height':'22px','padding-top':'22px'}).html("<span style='display:none'>"+percent+"</span>"+lan.index.memre_ok_1+"<br>"+memNull+"MB");
				}
				else{
					$(".mem-release").find('.mask').css({'color':'black','font-size':'14px'}).html("<span style='display:none'>"+percent+"</span>"+lan.index.memre_ok_2);
				}
